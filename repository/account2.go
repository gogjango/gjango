...
// NewAccountRepo2 returns an AccountRepo instance
func NewAccountRepo2(db orm.DB, log *zap.Logger) *AccountRepo2 {
	return &AccountRepo2{db, log}
}

// AccountRepo2 represents the client for the user table
type AccountRepo2 struct {
	db  orm.DB
	log *zap.Logger
}

// Create creates a new user in our database
func (a *AccountRepo2) Create(u *model.User) (*model.User, error) {
	user := new(model.User)
...
func (a *AccountRepo2) CreateAndVerify(u *model.User) (*model.Verification, error) {
	user := new(model.User)
	sql := `SELECT id FROM users WHERE username = ? OR email = ? OR (country_code = ? AND mobile = ?) AND deleted_at IS NULL`
	res, err := a.db.Query(user, sql, u.Username, u.Email, u.CountryCode, u.Mobile)
	if err != nil {
...
func (a *AccountRepo2) CreateWithMobile(u *model.User) error {
	user := new(model.User)
	sql := `SELECT id FROM users WHERE username = ? OR email = ? OR (country_code = ? AND mobile = ?) AND deleted_at IS NULL`
	res, err := a.db.Query(user, sql, u.Username, u.Email, u.CountryCode, u.Mobile)
	if err != nil {
...
func (a *AccountRepo2) ChangePassword(u *model.User) error {
	u.Update()
...
func (a *AccountRepo2) FindVerificationToken(token string) (*model.Verification, error) {
...
func (a *AccountRepo2) DeleteVerificationToken(v *model.Verification) error {
